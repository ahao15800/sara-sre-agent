#!/system/bin/sh
# SARA SRE Agent - Snapshot Engine (Refactored for KernelSU)
set -u
TIMESTAMP=$(date +%s)
OUTPUT_FILE="${SARA_SNAPSHOT_FILE:-/data/local/tmp/system_snapshot.json}"
mkdir -p "$(dirname "$OUTPUT_FILE")"

SUSFS_EXISTS=false; SUSFS_VERSION="null"; VFS_HIDING=0; SUSFS_SUS_PATH_COUNT=0
NAMESPACE_LEAK=false; INIT_MOUNTS_COUNT=0; SELF_MOUNTS_COUNT=0; MODULE_MOUNTS_COUNT=0
SELINUX_STATE="未知"; LSPOSED_DETECTED=false; BINDER_STATS_AVAILABLE=false; BINDER_TRANSACTION_COUNT=0
...
if [ -f "/proc/1/mounts" ] && [ -f "/proc/self/mounts" ]; then
    ...
    [ "$DIFF" -gt 15 ] || [ "$MODULE_MOUNTS_COUNT" -gt 0 ] && NAMESPACE_LEAK=true
fi

[ -f "/sys/fs/selinux/enforce" ] && ([ "$(cat /sys/fs/selinux/enforce)" -eq 1 ] && SELINUX_STATE="Enforcing" || SELINUX_STATE="Permissive")
...
{
  "timestamp": $TIMESTAMP,
  "kernel_susfs": { "active": $SUSFS_EXISTS, "version": "$SUSFS_VERSION", "vfs_hiding_enabled": $VFS_HIDING, "sus_path_count": $SUSFS_SUS_PATH_COUNT },
  "mount_namespace": { "leak_detected": $NAMESPACE_LEAK, "init_mounts_count": $INIT_MOUNTS_COUNT, "self_mounts_count": $SELF_MOUNTS_COUNT, "module_mounts_count": $MODULE_MOUNTS_COUNT },
  "middleware": { "selinux_state": "$SELINUX_STATE", "lsposed_detected": $LSPOSED_DETECTED },
  "binder": { "stats_available": $BINDER_STATS_AVAILABLE, "total_transactions": ${BINDER_TRANSACTION_COUNT:-0} },
